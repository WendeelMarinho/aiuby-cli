...
- `aiuby-guide` — lists commands and where to read docs, but as a flat catalog.
...
- User invokes `/aiuby-recipes` with or without a description.

### Do Not Use When

- User wants the task done now — route to the actual command, don't describe it.
- User wants deep docs for ONE command — use `aiuby-guide`.
...
5. **Where to read** — the `commands/<name>.md` path plus `/aiuby-guide <name>`.
...
Read full docs:
  commands/<cmd1>.md   (or: /aiuby-guide <cmd1>)
```

## Examples

**Catalog:** `/aiuby-recipes` → prints the family table and total count.

**Match:** `/aiuby-recipes plan a whole app upfront then auto-build with adversarial
...
**Match:** `/aiuby-recipes fix a bug in my Go service` → Best fit: `orch-fix-defect`
...
- Not per-command deep docs — that's `aiuby-guide`.
