  ...
  .then(raw => {
    try {
      ...
      if (hookEnabled('pre:bash:dev-server-block', ['standard', 'strict']) && process.platform !== 'win32') {
        ...
        if (hasBlockedDev) {
          console.error('[Aiuby] BLOCKED: Dev server must run in tmux for log access');
          console.error('[Aiuby] Use: tmux new-session -d -s dev "npm run dev"');
          process.exit(2);
        }
      }
      ...
      if (
        ...
        console.error('[Aiuby] Consider running in tmux for session persistence');
      }

      if (hookEnabled('pre:bash:git-push-reminder', ['strict']) && /\bgit\s+push\b/.test(cmd)) {
        console.error('[Aiuby] Review changes before push: git diff origin/main...HEAD');
      }
    } catch {
      // noop
    }
