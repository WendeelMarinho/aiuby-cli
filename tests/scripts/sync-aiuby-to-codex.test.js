/**
 * Source-level tests for scripts/sync-aiuby-to-codex.sh
 */
...
const scriptPath = path.join(__dirname, '..', '..', 'scripts', 'sync-aiuby-to-codex.sh');
...
function runTests() {
  console.log('\n=== Testing sync-aiuby-to-codex.sh ===\n');
  ...
  if (test('sync script avoids GNU-only grep -P parsing', () => {
    assert.ok(!source.includes('grep -oP'), 'sync-aiuby-to-codex.sh should remain portable across BSD and GNU environments');
  })) passed++; else failed++;
