/**
 * Tests for scripts/lib/resolve-aiuby-root.js
 *
...
const { resolveEccRoot, INLINE_RESOLVE } = require('../../scripts/lib/resolve-aiuby-root');
...
function runTests() {
  ...
  if (test('INLINE_RESOLVE delegates to committed resolver when env var is unset', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(resolverDir, 'resolve-aiuby-root.js'), `module.exports = { resolveEccRoot() { return 'delegated:' + process.env.INLINE_RESOLVE_MARKER; } };`);
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
        '-e', `console.log(${INLINE_RESOLVE})`,
      ], {
  ...
  if (test('INLINE_RESOLVE loads the committed resolver module from home base', () => {
    ...
    try {
      const resolverDir = path.join(homeDir, '.claude', 'scripts', 'lib');
      fs.mkdirSync(resolverDir, { recursive: true });
      fs.writeFileSync(path.join(resolverDir, 'resolve-aiuby-root.js'), `const assert = require('assert');
module.exports = { resolveEccRoot() { assert.strictEqual(process.env.HOME, ${JSON.stringify(homeDir)}); return 'module-loaded'; } };`);
  ...
  if (test('INLINE_RESOLVE bootstraps module from an exact plugin root when env unset', () => {
    ...
    try {
      ...
      fs.mkdirSync(resolverDir, { recursive: true });
      fs.writeFileSync(path.join(resolverDir, 'resolve-aiuby-root.js'), `module.exports = { resolveEccRoot() { return 'plugin-root'; } };`);
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
        '-e', `console.log(${INLINE_RESOLVE})`,
      ], {
  ...
  if (test('INLINE_RESOLVE bootstraps module from the versioned plugin cache when env unset', () => {
    ...
    try {
      ...
      fs.mkdirSync(resolverDir, { recursive: true });
      fs.writeFileSync(path.join(resolverDir, 'resolve-aiuby-root.js'), `module.exports = { resolveEccRoot() { return 'cache-root'; } };`);
      const { execFileSync } = require('child_process');
      const result = execFileSync('node', [
        '-e', `console.log(${INLINE_RESOLVE})`,
      ], {
