 ...
 * message through the canonical writer, the `aiuby-tui messages send` CLI. The CLI
...
// Proximity trigger type → aiuby-tui message kind (value_enum on `--kind`).
...
/**
 * Resolve the aiuby-tui binary: explicit override, env var, a built target in the
...
function resolveEccBin(deps = {}) {
  ...
  for (const rel of ['ecc2/target/release/aiuby-tui', 'ecc2/target/debug/aiuby-tui']) {
 ...
 * delivers via `aiuby-tui messages send`. Inject `runCommand(bin, args)` for tests.
 */
